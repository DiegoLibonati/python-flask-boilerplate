...
class TestNoteServiceAddNote:
    ...
    def test_raises_conflict_error_when_name_already_exists(self) -> None:
        ...
        with patch("src.services.note_service.NoteDAO.find_one_by_name", return_value={"_id": "1", "name": "existing"}):
            with pytest.raises(ConflictAPIError):
                NoteService.add_note(note)

    def test_conflict_error_has_correct_code(self) -> None:
        note: NoteModel = NoteModel(name="existing")
        with patch("src.services.note_service.NoteDAO.find_one_by_name", return_value={"name": "existing"}):
